--[[
    ════════════════════════════════════════════════════════════════
    [ HILICHURL HUB — OFFICIAL KEY SYSTEM LOADER ]
    Game: Blox Fruits (Sea Events, Leviathan & Automation)
    ════════════════════════════════════════════════════════════════
]]

local Config = {
    -- [1] PlatoBoost Public Service ID
    ServiceId       = 30571,

    -- [2] Anti-Bypass Variable
    Secret          = "HilichurlAuthSecret",
    
    -- [3] Cloudflare Worker Secure API Endpoint
...
local function generateNonce()
    local str = ""
    for _ = 1, 16 do str = str .. string.char(math.floor(math.random() * (122 - 97 + 1)) + 97) end
    return str
end
...
-------------------------------------------------------------------------------
--! CORE AUTHENTICATION (Platoboost Client Redeem + Cloudflare Script Delivery)
-------------------------------------------------------------------------------
local function VerifyKeyAndRun(keyString, onSuccess, onError)
    ...
    -- 1. Redeem trực tiếp từ máy Client lên Platoboost
    local response, err = safeRequest({
    ...
    if response and (response.StatusCode == 200 or response.StatusCode == 400) then
        ...
        if ok and decoded and decoded.success and decoded.data and decoded.data.valid and decoded.data.hash then
            ...
            -- 2. Gửi Chữ ký số về Cloudflare Worker để kéo source main.lua
